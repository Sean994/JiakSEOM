...
exports.searchRes = async (req, res, next) => {
  ...
  try {
    ...
    if (data.length === 0) {
      throw new Error(
        `Sorry! No Restaurant is found with this name : ${req.query.query}`
      );
    }
    res.status(200).json({
      status: 'success',
...
exports.searchItems = async (req, res, next) => {
  ...
  let test3 = test2.map((data) => data.toLowerCase());
  ...
  try {
    ...
    if (data.length === 0) {
      throw new Error(
        `Sorry! No item is found with this name : ${req.query.query}`
      );
    }
    const array = [];
    data.forEach((item) => {
